...
//   node ninsai-kakurenbo/tools/simtest.js [試合数] [難易度] [--fixed]（--fixed＝旧来の3体固定・省略時は39体からランダム）
...
for (let m = 0; m < N; m++) {
  ...
  const exposedAt = {};
  while (g.phase !== "finished" && ticks < 30 * 400) {
    Sim.step(g); ticks++;
...
console.log(`試合数 ${N} / 難易度 ${dif} / キャラ ${fixed ? "固定3体" : "39体ランダム"}`);
